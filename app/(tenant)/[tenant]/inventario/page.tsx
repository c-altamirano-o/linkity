...
export default function InventarioPage() {
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      {/* ── Topbar ──────────────────────────────────────────── */}
      <div className="bg-white border-b border-slate-200 px-3 sm:px-5 py-2.5 flex items-center gap-2 sm:gap-3 flex-wrap">
        <span className="text-sm font-medium text-slate-800 flex-1 min-w-0">Inventario</span>
        ...
        <div className="relative order-last sm:order-none w-full sm:w-auto">
          ...
          <input type="text" value={busqueda} onChange={e => setBusqueda(e.target.value)}
            ...
            className="w-full sm:w-48 pl-7 pr-3 py-2 border border-slate-200 rounded-lg text-xs bg-slate-50 focus:outline-none focus:ring-2 focus:ring-[#4F46E5]/20 focus:border-[#4F46E5]" />
        </div>

        <div className="flex items-center gap-2">
          <button className="flex items-center gap-1.5 px-2.5 py-2 border border-slate-200 rounded-lg text-xs text-slate-600 hover:bg-slate-50">
          ...
          <button className="flex items-center gap-1.5 px-2.5 sm:px-3 py-2 bg-[#4F46E5] hover:bg-[#4338CA] text-white rounded-lg text-xs font-medium transition-colors whitespace-nowrap">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-3 px-3 sm:px-5 py-3 bg-white border-b border-slate-200">
        {[
          ...
          <div key={m.label} className="bg-slate-50 rounded-lg p-2.5 sm:p-3">
            <p className="text-[9px] sm:text-[10px] text-slate-400 mb-1">{m.label}</p>
      ...
      {/* ── Filtros tabs ─────────────────────────────────────── */}
      <div className="flex gap-2 px-3 sm:px-5 py-2 bg-white border-b border-slate-100 overflow-x-auto">
        {filtrosTabs.map(tab => (
          ...
          <thead className="sticky top-0 bg-white border-b border-slate-200 z-10">
            <tr>
              {["Producto","Categoría","Stock actual","Stock mínimo","Precio venta","Costo","Acción"].map(h => (
                <th key={h} className="text-left text-[10px] font-medium text-slate-400 px-3 sm:px-4 py-2.5 whitespace-nowrap">
                  {h}
                </th>
              ))}
            </tr>
                  ...
                  <td className="px-3 sm:px-4 py-2.5">
                    <div className="flex items-center gap-2">
                      ...
                      <div className="min-w-0">
                        <p className="text-xs font-medium text-slate-800 truncate max-w-[140px]">{p.nombre}</p>
                  ...
                  <td className="px-3 sm:px-4 py-2.5">
                    <div className="flex items-center gap-2">
                      ...
                      <span className={`text-xs font-medium ${
                        status==="out"?"text-red-600":status==="low"?"text-amber-600":"text-slate-800"
                      }`}>
                        {p.stock}
                      </span>
                    </div>
                  </td>
                  <td className="px-3 sm:px-4 py-2.5 text-xs text-slate-500">{p.minStock}</td>
                  <td className="px-3 sm:px-4 py-2.5 text-xs font-medium text-slate-800">{formatMXN(p.precio)}</td>
                  <td className="px-3 sm:px-4 py-2.5 text-xs text-slate-500">{formatMXN(p.costo)}</td>
                  <td className="px-3 sm:px-4 py-2.5">
      ...
      {modalAjuste && (
        <div className="fixed inset-0 bg-black/40 flex items-end sm:items-center justify-center z-50">
          <div className="bg-white rounded-t-2xl sm:rounded-2xl p-5 w-full sm:w-80 shadow-xl">
            <h2 className="text-sm font-semibold text-slate-800 mb-1">Ajuste de stock</h2>
            <p className="text-xs text-slate-400 mb-4 truncate">{modalAjuste.nombre}</p>
            ...
            <div className="flex items-center justify-between bg-slate-50 rounded-lg p-3 mb-4">
              ...
              <span className="text-sm font-semibold text-slate-800">{modalAjuste.stock}</span>
            </div>
            ...
            <div className="mb-4">
              ...
              <input type="number" value={ajusteCantidad} onChange={e => setAjusteCantidad(e.target.value)}
                placeholder="0"
                className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#4F46E5]/20 focus:border-[#4F46E5]" />
            </div>
            ...
            <div className="flex gap-2">
              ...
              <button onClick={() => setModalAjuste(null)}
                className="flex-1 py-2 bg-[#4F46E5] hover:bg-[#4338CA] text-white rounded-lg text-xs font-medium transition-colors">
                Guardar
              </button>
            </div>
          </div>
