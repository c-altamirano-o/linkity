...
export default function CatalogoPage() {
      ...
      <div className="flex items-center justify-between px-3 py-3 border-b border-slate-100">
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => setSidebarMovil(false)}
            className="md:hidden w-6 h-6 flex items-center justify-center text-slate-400 hover:text-slate-600">
            <X className="w-4 h-4" />
          </button>
        </div>
      </div>
            ...
            <div key={tipo} className="mb-3">
              <div
                className={`flex items-center gap-2 px-2 py-1.5 rounded-lg mb-1 cursor-pointer ${
                  tipoActivo===tipo && tabActivo==="catalogo" ? cfg.bg : "bg-slate-50"
                }`}
                onClick={() => { setTipoActivo(tipo); setCategoriaActiva(categorias[tipo][0]); setTabActivo("catalogo"); setSidebarMovil(false); }}>
              ...
              {tipoActivo===tipo && tabActivo==="catalogo" && categorias[tipo].map(cat => (
                <div key={cat}
                  onClick={() => { setCategoriaActiva(cat); setSidebarMovil(false); }}
  ...
  return (
    <div className="flex h-full">
      ...
      <div className="hidden md:flex w-48 flex-col bg-white border-r border-slate-200 flex-shrink-0">
        {sidebarContent}
      </div>
      ...
      {sidebarMovil && (
        <>
          ...
          <div className="md:hidden fixed left-0 top-0 bottom-0 w-56 bg-white z-50 flex flex-col shadow-xl">
            {sidebarContent}
          </div>
        </>
      )}
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <div className="flex bg-white border-b border-slate-200 px-2 sm:px-4 overflow-x-auto">
          ...
          <button onClick={() => setSidebarMovil(true)}
            className="md:hidden flex items-center justify-center w-8 h-full mr-1 text-slate-500 flex-shrink-0">
        ...
        {tabActivo==="catalogo" && (
          <div className="md:hidden flex gap-1.5 px-3 py-2 border-b border-slate-100 overflow-x-auto bg-white flex-shrink-0">
            {categorias[tipoActivo].map(cat => (
              <button key={cat} onClick={() => setCategoriaActiva(cat)}
                ...
                {cat} <span className="opacity-60">({conteoCat(tipoActivo, cat)})</span>
              </button>
            ))}
          </div>
        )}
        ...
        {tabActivo==="catalogo" && (
          <>
            <div className="bg-white border-b border-slate-200 px-3 sm:px-4 py-2 flex items-center gap-2 sm:gap-3">
              ...
              <button className="flex items-center gap-1.5 px-2.5 sm:px-3 py-2 border border-slate-200 rounded-lg text-xs text-slate-600 hover:bg-slate-50 transition-colors flex-shrink-0">
                <SlidersHorizontal className="w-3 h-3" />
                <span className="hidden sm:inline">Filtros</span>
              </button>
            </div>

            {/* Grid: 2 cols móvil → 3 → 4 en desktop grande */}
            <div className="flex-1 overflow-y-auto p-3 sm:p-4 grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-4 gap-2 sm:gap-3 content-start">
              {productosFiltrados.map(p => (
                <div key={p.id} className="bg-white border border-slate-200 rounded-xl overflow-hidden hover:border-[#4F46E5]/40 hover:shadow-sm transition-all cursor-pointer">
                  <div className="h-14 sm:h-16 bg-slate-50 flex items-center justify-center text-2xl border-b border-slate-100">
                    {p.emoji}
                  </div>
                  <div className="p-2.5 sm:p-3">
                    <p className="text-xs font-medium text-slate-800 leading-tight mb-1 line-clamp-2">{p.nombre}</p>
                    <p className="text-[9px] text-slate-400 mb-2 truncate">{p.sku}</p>
                    <div className="flex items-center justify-between flex-wrap gap-1">
                      <span className="text-xs font-bold text-[#4F46E5]">{formatMXN(p.precio)}</span>
                      {stockBadge(p.stock, p.tipo)}
                    </div>
                    {p.tipo!=="servicios" && p.costo>0 && (
                      <p className="text-[9px] text-slate-400 mt-1">Costo: {formatMXN(p.costo)}</p>
                    )}
                  </div>
                </div>
              ))}
              <button className="flex flex-col items-center justify-center border border-dashed border-slate-200 rounded-xl hover:border-[#4F46E5]/40 hover:bg-slate-50 transition-all min-h-[130px] sm:min-h-[140px]">
        ...
        {tabActivo==="topventas" && (
          <div className="flex-1 overflow-y-auto p-3 sm:p-4">
            ...
            <div className="flex flex-col sm:flex-row sm:flex-wrap items-start sm:items-center gap-2 sm:gap-3 mb-4 p-3 bg-white border border-slate-200 rounded-xl">

              {/* Sucursal */}
              <div className="flex items-center gap-2 w-full sm:w-auto">
                <Building2 className="w-4 h-4 text-slate-400 flex-shrink-0" />
                <span className="text-xs text-slate-500 flex-shrink-0">Sucursal:</span>
                <select value={sucursal} onChange={e => setSucursal(e.target.value)}
                  className="flex-1 sm:flex-none px-2 py-1.5 border border-slate-200 rounded-lg text-xs bg-slate-50 focus:outline-none focus:border-[#4F46E5]">
            ...
            {/* Gráfica + Ranking — apilados en móvil, lado a lado en desktop */}
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-3 sm:gap-4">

              {/* Gráfica de barras */}
              <div className="bg-white border border-slate-200 rounded-xl p-4">
                <p className="text-sm font-medium text-slate-800 mb-1">Top 5 por ingresos</p>
                <p className="text-xs text-slate-400 mb-3 capitalize">{sucursal} · {periodoLabel[periodo]}</p>
                <ResponsiveContainer width="100%" height={180}>
                  <BarChart data={topVentasData} layout="vertical"
                    ...
                    <Bar dataKey="total" radius={[0,4,4,0]}
                      label={{ position:"right", fontSize:9, fill:"#64748B", formatter:(v:any)=>formatMXN(Number(v)) }}>
                      {topVentasData.map((entry, i) => <Cell key={i} fill={entry.color} />)}
                    </Bar>
                  </BarChart>
                </ResponsiveContainer>
              </div>
                ...
                <div className="divide-y divide-slate-50">
                  {topVentasData.map((item, i) => (
                    <div key={i} className="flex items-center gap-3 px-4 py-3 hover:bg-slate-50 transition-colors">
                      ...
                      <span className="text-xs font-semibold text-[#4F46E5] flex-shrink-0">{formatMXN(item.total)}</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div className="mt-3 flex items-start sm:items-center gap-2 px-4 py-2.5 bg-emerald-50 border border-emerald-200 rounded-xl">
              ...
              <p className="text-xs text-emerald-700">
                Mostrando datos de <strong>{sucursal}</strong> · {periodoLabel[periodo]} · Total generado: <strong>{formatMXN(totalTop)}</strong>
              </p>
            </div>
          </div>
        )}
