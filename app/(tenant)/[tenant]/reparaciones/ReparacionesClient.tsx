...
function VistaTienda({
  ...
  const tiendaReps = reparaciones.filter((r) => {
    ...
    // "Pendientes" solo mostraba equipos en SHOP_READY/SHOP_RETURN (ya
    // trasladados a tienda) — un equipo recién recibido (RECEIVED) o
    // todavía en diagnóstico/espera de piezas/reparación en el taller
    // (DIAGNOSING/WAITING_PARTS/IN_REPAIR/READY/WORKSHOP_READY/
    // WORKSHOP_RETURN) desaparecía de "Pendientes" aunque el negocio
    // todavía lo tuviera pendiente de principio a fin — solo aparecía en
    // "Todo". Regla correcta, tal cual la pidió Carlos: "mientras un
    // equipo no sea entregado, debe aparecer como pendiente" — es decir,
    // cualquier estado que no sea DELIVERED (entregado) ni CANCELLED
    // (cancelado, ya no hay nada pendiente que hacer con él).
    const esPendiente = r.estado !== "DELIVERED" && r.estado !== "CANCELLED";
