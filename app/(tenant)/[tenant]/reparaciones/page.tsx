...
import {
  ...
  Phone, CheckCircle, AlertCircle, User
} from "lucide-react";
...
const reparaciones = [
  {
    ...
    estado: "listo_tienda", prioridad: "alta", costoEstimado: 1800, costoFinal: 1800,
  ...
  },
  {
    ...
    estado: "listo_taller", prioridad: "normal", costoEstimado: 950, costoFinal: 950,
  ...
  },
  {
    ...
    estado: "listo_tienda", prioridad: "normal", costoEstimado: 650, costoFinal: 650,
  ...
  },
  {
    ...
    estado: "devolucion_tienda", prioridad: "baja", costoEstimado: 0, costoFinal: 0,
    fechaRecibido: "28 mayo 2026", fechaEstimada: "30 mayo 2026",
    tecnico: "Juan Pérez", whatsappEnviado: false, publicToken: "REP-0039-jkl012",
    progreso: 70,
    historial: [
  ...
  {
    id: 5, folio: "REP-0038", cliente: "Jorge Pérez", telefono: "614 567 8901",
    iniciales: "JP", marca: "Apple", modelo: "iPhone 14", falla: "Sin señal",
    estado: "en_reparacion", prioridad: "alta", costoEstimado: 1200, costoFinal: 0,
    fechaRecibido: "24 mayo 2026", fechaEstimada: "26 mayo 2026",
    tecnico: "Juan Pérez", whatsappEnviado: true, publicToken: "REP-0038-mno345",
    progreso: 40,
    historial: [
...
const esDevolución = (estado: string) =>
...
function VistaTienda() {
  ...
  const tiendaReps = reparaciones.filter((r) => {
    ...
    const matchSearch = r.cliente.toLowerCase().includes(busqueda.toLowerCase()) ||
  ...
  const esDev = seleccionada.estado === "devolucion_tienda";
          ...
          {tiendaReps.length === 0 ? (
            ...
            <div key={rep.id} onClick={() => setSeleccionada(rep)}
              ...
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className={`text-[9px] font-medium px-1.5 py-0.5 rounded-full ${estadoConfig[rep.estado].badge}`}>
        ...
        <div className="bg-white border-b border-slate-200 px-5 py-4">
          <div className="flex items-start justify-between mb-3">
            <div className="flex items-center gap-3">
              <div className={`w-10 h-10 rounded-xl flex items-center justify-center text-sm font-semibold flex-shrink-0 ${estadoConfig[seleccionada.estado].avatar}`}>
            ...
            <div className="flex gap-2">
              <button className="flex items-center gap-1.5 px-3 py-1.5 bg-[#25D366] hover:bg-[#22c35e] text-white rounded-lg text-xs font-medium transition-colors">
                <Phone className="w-3 h-3" /> Avisar cliente
              </button>
              {(seleccionada.estado === "listo_tienda" || seleccionada.estado === "devolucion_tienda") && (
                <button className="flex items-center gap-1.5 px-3 py-1.5 bg-[#4F46E5] hover:bg-[#4338CA] text-white rounded-lg text-xs font-medium transition-colors">
                  <CheckCircle className="w-3 h-3" /> Marcar entregado
                </button>
              )}
            </div>
          </div>
          ...
          <div className={`flex items-center gap-3 px-4 py-3 rounded-xl border ${
            esDev ? "bg-amber-50 border-amber-200" : "bg-emerald-50 border-emerald-200"
          }`}>
            {esDev
            ...
            <div>
              <p className={`text-xs font-semibold ${esDev ? "text-amber-700" : "text-emerald-700"}`}>
                {esDev ? "Equipo para devolver al cliente" : "Equipo listo para entregar"}
              </p>
              <p className={`text-[10px] mt-0.5 ${esDev ? "text-amber-600" : "text-emerald-600"}`}>
                {seleccionada.modelo} ·{" "}
                {esDev
                  ? "No fue posible realizar la reparación"
                  : `Reparación completada · Costo final: ${formatMXN(seleccionada.costoFinal)}`
                }
              </p>
            </div>
          </div>
          ...
          <div className="bg-white border border-slate-200 rounded-xl p-4">
            ...
            <div className="grid grid-cols-2 gap-2">
              {[
                ...
                { label: "Costo final",   value: seleccionada.costoFinal > 0 ? formatMXN(seleccionada.costoFinal) : esDev ? "Sin cargo" : "Por definir", color: esDev ? "text-amber-600" : "text-[#4F46E5]" },
...
function VistaAdmin() {
  ...
  const devolucion = esDevolución(seleccionada.estado);
          ...
          {filtradas.map((rep) => (
            <div key={rep.id} onClick={() => setSeleccionada(rep)}
              ...
              <div className="h-1 bg-slate-100 rounded-full overflow-hidden">
                <div className={`h-full rounded-full transition-all ${esDevolución(rep.estado) ? "bg-orange-400" : "bg-gradient-to-r from-[#4F46E5] to-[#06B6D4]"}`}
        ...
        <div className="bg-white border-b border-slate-200 px-5 py-3">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm font-semibold text-slate-800">
                  {seleccionada.folio}
                  <span className="text-xs font-normal text-slate-500 ml-2">— {seleccionada.marca} {seleccionada.modelo}</span>
                </p>
                <p className="text-xs text-slate-400">{seleccionada.cliente} · {seleccionada.telefono} · Recibido {seleccionada.fechaRecibido}</p>
              </div>
            </div>
          ...
          <div className="flex items-center">
            {flujoSteps.map((step, i) => {
              ...
              const isDevolucionStep = devolucion && (isCurrent || isDone) && i >= 1;
              const stepColor = isDevolucionStep ? "bg-orange-500 border-orange-500" : "bg-[#4F46E5] border-[#4F46E5]";
              const lineColor = isDevolucionStep ? "bg-orange-400" : "bg-[#4F46E5]";
              const labelColor = isCurrent
                ? (devolucion ? "text-orange-500" : "text-[#4F46E5]")
                : isDone ? "text-[#4F46E5]" : "text-slate-400";
                ...
                <div key={step.key} className="flex items-center flex-1 last:flex-none">
                  <div className="flex flex-col items-center">
                    <div className={`w-7 h-7 rounded-full flex items-center justify-center border-2 transition-all ${
                      isDone ? stepColor :
                      isCurrent ? `bg-white ${devolucion ? "border-orange-500" : "border-[#4F46E5]"}` :
                      "bg-white border-slate-200"
                    }`}>
                      {isDone
                        ? <Check className="w-3 h-3 text-white" />
                        : <Icono className={`w-3 h-3 ${isCurrent ? (devolucion ? "text-orange-500" : "text-[#4F46E5]") : "text-slate-300"}`} />
                      }
                    </div>
                    <div className="flex flex-col items-center mt-1">
                      <span className={`text-[9px] whitespace-nowrap font-medium ${labelColor}`}>{step.label}</span>
                      {(step.key === "taller" || step.key === "tienda") && isCurrent && (
                        <span className={`text-[8px] font-semibold ${devolucion ? "text-orange-500" : "text-emerald-500"}`}>
                          {devolucion ? "Devolución" : "Listo"}
                        </span>
                      )}
                    </div>
                  </div>
          ...
          <div className="col-span-2 bg-emerald-50 border border-emerald-200 rounded-xl p-4">
            <div className="flex items-center gap-3 mb-3">
              ...
              {seleccionada.whatsappEnviado && (
                <button className="px-3 py-1.5 bg-[#25D366] hover:bg-[#22c35e] text-white text-xs font-medium rounded-lg">Reenviar</button>
              )}
            </div>
            {seleccionada.whatsappEnviado ? (
              <>
                <div className="bg-[#DCF8C6] rounded-lg p-3 text-xs text-slate-800 leading-relaxed mb-2 max-w-lg">
                  Hola {seleccionada.cliente.split(" ")[0]} 👋, tu <strong>{seleccionada.modelo}</strong> está siendo atendido. Puedes ver el estado en tiempo real aquí:
                </div>
                <div className="flex items-center gap-2 bg-white rounded-lg px-3 py-2 max-w-lg border border-emerald-200">
                  ...
                  <span className="text-xs text-[#4F46E5] flex-1">linkity.mx/rep/{seleccionada.publicToken}</span>
                  <button onClick={copiarLink} className="text-[10px] text-slate-400 hover:text-slate-600 flex items-center gap-1">
...
export default function ReparacionesPage() {
  // DEMO: simula el rol del usuario — en producción vendrá de la sesión real
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="bg-amber-50 border-b border-amber-200 px-4 py-1.5 flex items-center gap-3">
        <span className="text-[10px] text-amber-700 font-medium flex items-center gap-1">
          <User className="w-3 h-3" /> Simulador de rol (demo):
        </span>
        ...
        <span className="text-[10px] text-amber-500 ml-auto">Esta barra desaparece con auth real</span>
      </div>
