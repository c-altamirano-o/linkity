...
import { Search, Plus, Edit, ShoppingCart, Shield, Wrench, Phone } from "lucide-react";
    ...
    <div className="flex h-full">
      ...
      <div className="w-72 flex flex-col bg-white border-r border-slate-200 flex-shrink-0">
        <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100">
          <span className="text-[13px] font-medium text-slate-800">Clientes</span>
        ...
        <div className="flex-1 overflow-y-auto">
          {clientesFiltrados.map((c) => (
            <div key={c.id} onClick={() => setSeleccionado(c)}
              ...
              <div className={`w-9 h-9 rounded-full flex items-center justify-center text-[11px] font-semibold flex-shrink-0 ${c.avatarBg} ${c.avatarColor}`}>
                {c.iniciales}
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-[12px] font-medium text-slate-800">{c.nombre}</p>
        ...
        <div className="bg-white border-b border-slate-200 px-5 py-4">
          <div className="flex items-start justify-between mb-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-[15px] font-semibold text-slate-800">{seleccionado.nombre}</p>
            ...
            <div className="flex gap-2">
              ...
              <button className="flex items-center gap-1.5 px-3 py-1.5 bg-[#25D366] hover:bg-[#22c35e] text-white rounded-lg text-[11px] font-medium transition-colors">
                <span className="text-[11px] font-bold">W</span> WhatsApp
              </button>
              <button className="flex items-center gap-1.5 px-3 py-1.5 bg-[#4F46E5] hover:bg-[#4338CA] text-white rounded-lg text-[11px] font-medium transition-colors">
                <ShoppingCart className="w-3 h-3" /> Nueva venta
              </button>
            </div>
          </div>
          ...
          <div className="grid grid-cols-4 gap-3">
            {[
              ...
              <div key={s.label} className={`rounded-lg p-3 ${s.warning ? "bg-amber-50" : "bg-slate-50"}`}>
                <p className="text-[9px] text-slate-400 mb-1">{s.label}</p>
                <p className={`text-[15px] font-semibold ${s.warning ? "text-amber-600" : "text-slate-800"}`}>{s.value}</p>
        ...
        <div className="flex-1 overflow-y-auto p-4">
          ...
          {seleccionado.garantiasActivas > 0 && (
            <div className="flex items-center gap-3 bg-amber-50 border border-amber-200 rounded-xl p-3 mb-4">
              ...
              <div className="flex-1">
                <p className="text-[11px] font-medium text-amber-800">{seleccionado.garantiasActivas} garantía{seleccionado.garantiasActivas > 1 ? "s" : ""} activa{seleccionado.garantiasActivas > 1 ? "s" : ""}</p>
          ...
          <div className="space-y-2">
            {historialFiltrado.length === 0 ? (
              <div className="text-center py-8 text-slate-400 text-[12px]">Sin registros en esta categoría</div>
              ...
              <div key={i} className="flex items-center gap-3 p-3 bg-white border border-slate-200 rounded-xl hover:border-slate-300 transition-colors">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-[12px] font-medium text-slate-800">{h.titulo}</p>
                  <p className="text-[10px] text-slate-400">{h.folio} · {h.fecha}</p>
                </div>
                <span className={`text-[9px] font-medium px-2 py-0.5 rounded-full ${estadoConfig[h.estado]?.classes}`}>
                  {estadoConfig[h.estado]?.label}
                </span>
                <span className="text-[12px] font-semibold text-slate-800 min-w-[60px] text-right">
