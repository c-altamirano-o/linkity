...
export default function CajaPage() {
  ...
  const exportarXLSX = async () => {
    ...
    const addMergedRow = (range: string, text: string, bg: string, fontColor: string, size: number, bold = false, align: any = "center") => {
      ws.mergeCells(range);
    ...
    addMergedRow("A1:G1", "Cell Express — Reporte de Caja", PU, "FFFFFF", 14, true);
    ws.getRow(1).height = 28;
    addMergedRow("A2:G2", `Periodo: ${periodoLabel[periodo]}`, LP, "374151", 10);
    addMergedRow("A3:G3", `Generado el: ${fechaHoy}`, "F1F5F9", "94A3B8", 9);
    ws.addRow([]);
    addMergedRow("A5:G5", "RESUMEN DEL PERIODO", "1E293B", "FFFFFF", 10, true, "left");
    ...
    addMergedRow(`A${nr}:G${nr}`, "DETALLE DE MOVIMIENTOS", "1E293B", "FFFFFF", 10, true, "left");
  ...
  const exportarPNG = () => {
    ...
    const C = {
      purple: "#4F46E5", dark: "#0F172A", gray: "#F8FAFC",
      green:  "#16A34A", red: "#DC2626",  slate: "#64748B",
      border: "#E2E8F0", white: "#FFFFFF", subtext: "#94A3B8",
    };
    ...
    ctx.fillText("Cell Express — Reporte de Caja", 40, 44);
    ...
    movsFiltrados.forEach((mov, i) => {
      ...
      ctx.fillText(mov.fecha, cols[1].x + 10, y + 26);
      ctx.fillText(mov.hora,  cols[2].x + 10, y + 26);
      ctx.fillStyle = C.dark;
    ...
    ctx.fillText("Generado por Linkity Solutions · linkity.mx · Documento no fiscal", 40, fy + 22);
    ctx.fillText(fechaHoy, W - 40 - ctx.measureText(fechaHoy).width, fy + 22);

    canvas.toBlob(blob => {
      const url = URL.createObjectURL(blob!);
      const a   = document.createElement("a");
      a.href = url; a.download = nombreArchivo("png"); a.click();
      URL.revokeObjectURL(url);
    }, "image/png");
    setMostrarExportMenu(false);
  };
  ...
  const exportarPDF = async () => {
    ...
    const PU: RGB = [79,70,229],   DK: RGB = [15,23,42],   GN: RGB = [22,163,74];
    const RD: RGB = [220,38,38],   GR: RGB = [248,250,252], LG: RGB = [226,232,240];
    const WH: RGB = [255,255,255], SL: RGB = [100,116,139];
    ...
    doc.text("Cell Express — Reporte de Caja", 28, 26);
    doc.setFontSize(9);
    doc.setFont("helvetica", "normal");
    doc.setTextColor(200,200,230);
    doc.text(`Periodo: ${periodoLabel[periodo]}  ·  Generado el: ${fechaHoy}`, 28, 48);
    const folio = `Folio: RPT-${new Date().toISOString().slice(0,10).replace(/-/g,"")}`;
    doc.text(folio, W - 28 - doc.getTextWidth(folio), 48);

    // KPIs
    const kpiY = 78;
    const kpiW = (W - 56 - 3*10) / 4;
    [
    ...
    movsFiltrados.forEach((mov, i) => {
      ...
      doc.setTextColor(...SL);
      doc.text(mov.fecha, cols[1].x+6, y+14);
      doc.text(mov.hora,  cols[2].x+6, y+14);
      doc.setTextColor(...DK);
    ...
    [
      ...
      { label:"NETO",           v: ingresos - egresos, c: (ingresos-egresos)>=0 ? GN : RD },
    ].forEach(t => {
      doc.setFillColor(...WH);
      doc.setDrawColor(...LG);
    ...
    for (let p=1; p<=totalPgs; p++) {
      ...
      doc.setFont("helvetica","normal");
      doc.setTextColor(148,163,184);
      doc.text("Generado por Linkity Solutions · linkity.mx · Documento no fiscal", 28, PH-14);
  ...
  return (
    <div className="flex h-full">
      ...
      {/* Panel izquierdo */}
      <div className="w-64 flex flex-col bg-white border-r border-slate-200 flex-shrink-0">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <div className="bg-white border-b border-slate-200 px-4 py-2.5 flex items-center gap-3 flex-wrap">
          <span className="text-sm font-medium text-slate-800 whitespace-nowrap">Detalle de ventas</span>
          ...
          <div className="flex gap-2 ml-auto items-center">
            ...
            <div className="relative" ref={exportMenuRef}>
              <button onClick={() => setMostrarExportMenu(!mostrarExportMenu)}
                className="flex items-center gap-1.5 px-3 py-1.5 bg-[#4F46E5] hover:bg-[#4338CA] text-white rounded-lg text-xs font-medium transition-colors">
        ...
        {/* Métricas */}
        <div className="grid grid-cols-4 gap-3 px-4 py-3 bg-white border-b border-slate-200">
          {[
        ...
        <div className="flex-1 overflow-auto">
          <table className="w-full">
            <thead className="sticky top-0 bg-white border-b border-slate-200 z-10">
              <tr>
                {["Folio · Hora", "Concepto", "Método", "Tipo", "Monto"].map((h, i) => (
                  <th key={h} className={`text-left text-[10px] font-medium text-slate-400 px-4 py-2.5 whitespace-nowrap ${i === 4 ? "text-right" : ""}`}>
                    {h}
                  </th>
                ))}
              </tr>
              ...
              {movsFiltrados.map((mov) => (
                <tr key={mov.id} className="border-b border-slate-50 hover:bg-slate-50 transition-colors">
                  <td className="px-4 py-2.5">
                    ...
                    <p className="text-[9px] text-slate-400">{mov.hora} · {mov.fecha}</p>
                  </td>
                  <td className="px-4 py-2.5"><p className="text-xs text-slate-800">{mov.concepto}</p></td>
                  <td className="px-4 py-2.5">
