...
import {
  ...
  CheckCircle, RotateCcw, Receipt, Building2, X, Phone, Settings, ChevronLeft
} from "lucide-react";
import {
...
export default function TenantDashboard() {
  ...
  function TablaVentas() {
    return (
      <>
        <div className="grid grid-cols-[70px_1fr_80px_65px] sm:grid-cols-[80px_1fr_90px_75px] px-4 py-2 bg-slate-50 border-b border-slate-100 sticky top-0">
          {["Folio · Hora","Artículos","Método","Total"].map((h,i) => (
            <p key={h} className={`text-[10px] font-medium text-slate-400 ${i===3?"text-right":""}`}>{h}</p>
          ))}
        </div>
        {ventasHoy.map(v => (
          <div key={v.folio} className="grid grid-cols-[70px_1fr_80px_65px] sm:grid-cols-[80px_1fr_90px_75px] px-4 py-3 border-b border-slate-50 hover:bg-slate-50 items-center">
            <div>
              <p className="text-xs font-semibold text-[#4F46E5]">{v.folio}</p>
              <p className="text-[10px] text-slate-400">{v.hora}</p>
            </div>
            <div>
              <p className="text-xs text-slate-800 truncate">{v.articulos}</p>
  ...
  const renderModal = () => {
    ...
    const configs: Record<NonNullable<ModalType>,{
        ...
        content:(
          <>
            ...
            {reparacionesData.map(r=>(
              <div key={r.folio} className="grid grid-cols-[80px_1fr_90px_75px] px-4 py-3 border-b border-slate-50 hover:bg-slate-50 items-center">
                <div>
                ...
                <span className={`text-[9px] font-medium px-1.5 py-0.5 rounded-full w-fit ${estadoConfig[r.estado]?.classes}`}>
      ...
      listos: {
        ...
        content:(
          <>
            <div className="grid grid-cols-[80px_1fr_90px_70px] px-4 py-2 bg-slate-50 border-b border-slate-100 sticky top-0">
              {["Folio","Cliente · Equipo","Costo","Espera"].map(h=>(
                <p key={h} className="text-[10px] font-medium text-slate-400">{h}</p>
              ))}
            </div>
            {equiposListos.map(e=>(
              <div key={e.folio} className="grid grid-cols-[80px_1fr_90px_70px] px-4 py-3 border-b border-slate-50 hover:bg-slate-50 items-center">
                <div>
      ...
      devoluciones: {
        ...
        content:(
          <>
            <div className="grid grid-cols-[80px_1fr_1fr] px-4 py-2 bg-slate-50 border-b border-slate-100 sticky top-0">
              {["Folio","Cliente · Equipo","Razón · Espera"].map(h=>(
                <p key={h} className="text-[10px] font-medium text-slate-400">{h}</p>
              ))}
            </div>
            {equiposDev.map(e=>(
              <div key={e.folio} className="grid grid-cols-[80px_1fr_1fr] px-4 py-3 border-b border-slate-50 hover:bg-slate-50 items-start">
                <div>
                  <p className="text-xs font-semibold text-[#4F46E5]">{e.folio}</p>
    ...
    return (
      <div className="fixed inset-0 bg-slate-900/50 flex items-end sm:items-center justify-center z-50 p-0 sm:p-4"
        ...
        <div className="bg-white rounded-t-2xl sm:rounded-2xl w-full sm:max-w-2xl max-h-[90vh] sm:max-h-[80vh] flex flex-col shadow-2xl"
          ...
          <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100 flex-shrink-0">
            ...
            <button onClick={() => setModalAbierto(null)}
              className="w-7 h-7 bg-slate-100 hover:bg-slate-200 rounded-lg flex items-center justify-center flex-shrink-0">
              <X className="w-4 h-4 text-slate-500" />
            </button>
          </div>
          <div className="grid grid-cols-3 gap-2 sm:gap-3 px-4 sm:px-5 py-3 border-b border-slate-100 flex-shrink-0">
            {cfg.stats.map(s => (
              <div key={s.label} className="bg-slate-50 rounded-xl p-2 sm:p-3 text-center">
  ...
  return (
    <div className="p-3 sm:p-4 space-y-3 sm:space-y-4 overflow-y-auto h-full">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-base sm:text-lg font-semibold text-slate-800">Buenos días 👋</h1>
          <p className="text-xs text-slate-400 mt-0.5">Miércoles 28 de mayo, 2026</p>
        </div>
        <div className="text-right">
          <p className="text-[10px] text-slate-400">Total semana</p>
          <p className="text-sm sm:text-base font-bold text-slate-800">{formatMXN(totalSemana)}</p>
        </div>
      </div>

      {/* ── Métricas 5 fichas ─────────────────────────────── */}
      {/* Mobile: 2 cols → sm: 3 cols → xl: 5 cols */}
      <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-5 gap-2 sm:gap-3">
        {([
          ...
          <div key={m.label} className="bg-white border border-slate-200 rounded-xl p-3 hover:border-slate-300 transition-colors">
            <div className="flex items-center justify-between mb-2">
              <p className="text-[10px] sm:text-xs text-slate-500 leading-tight">{m.label}</p>
            ...
            <p className="text-xl sm:text-[22px] font-semibold text-slate-800 leading-none mb-1">{m.value}</p>
      ...
      {/* ── Gráficas: área + pie ──────────────────────────── */}
      {/* Mobile: 1 col, Desktop: 2/3 + 1/3 */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-3 sm:gap-4">

        <div className="lg:col-span-2 bg-white border border-slate-200 rounded-xl p-3 sm:p-4">
          <div className="flex items-center justify-between mb-3">
            ...
            <div className="flex items-center gap-2 sm:gap-4">
              <div className="flex items-center gap-1"><div className="w-2 h-2 rounded-full bg-[#4F46E5]" /><span className="text-[10px] sm:text-xs text-slate-500">Ventas</span></div>
              <div className="flex items-center gap-1"><div className="w-2 h-2 rounded-full bg-[#06B6D4]" /><span className="text-[10px] sm:text-xs text-slate-500">Rep.</span></div>
              <div className="flex items-center gap-1"><div className="w-3 h-0 border-t-2 border-dashed border-emerald-500" /><span className="text-[10px] sm:text-xs text-slate-500">Total</span></div>
            </div>
          </div>
          <ResponsiveContainer width="100%" height={140}>
            <AreaChart data={ventasSemana} margin={{ top:5, right:5, bottom:0, left:0 }}>
              ...
              <Area type="monotone" dataKey="ventas"       stroke="#4F46E5" strokeWidth={2} fill="url(#cV)" />
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-3 sm:p-4">
          ...
          <p className="text-xs text-slate-400 mb-2">Este mes</p>
          {/* En móvil: pie + leyenda horizontal */}
          <div className="flex flex-col sm:block">
            ...
            {/* Leyenda: en móvil 2 cols, en desktop 1 col */}
            <div className="grid grid-cols-2 sm:grid-cols-1 gap-1 mt-1">
              {categoriasVisibles.map(cat => (
                <div key={cat.name} className="flex items-center justify-between">
                  <div className="flex items-center gap-1.5">
                    <div className="w-2 h-2 rounded-full flex-shrink-0" style={{background:cat.color}} />
                    <span className="text-[10px] sm:text-xs text-slate-600 truncate">{cat.name}</span>
                  </div>
                  <span className="text-[10px] sm:text-xs font-medium text-slate-700 ml-1">{cat.value}%</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="bg-white border border-slate-200 rounded-xl overflow-hidden">
          ...
          <div className="divide-y divide-slate-50">
            {reparacionesData.slice(0,4).map(r => (
              <div key={r.folio} className="flex items-center gap-3 px-4 py-2.5 hover:bg-slate-50">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[10px] text-slate-400 truncate">{r.cliente}</p>
                </div>
                <span className={`text-[9px] font-medium px-1.5 py-0.5 rounded-full whitespace-nowrap ${estadoConfig[r.estado]?.classes}`}>
                  {estadoConfig[r.estado]?.label}
                </span>
              </div>
            ))}
      ...
      {multiSucursal && (
        <div className="bg-white border border-slate-200 rounded-xl overflow-hidden">
          <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100">
            ...
            <span className="text-xs text-slate-400 hidden sm:block">Hoy · 28 mayo 2026</span>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 divide-y md:divide-y-0 md:divide-x divide-slate-100">
            {sucursales.map(suc => (
              <div key={suc.id} className={`border-t-2 ${suc.color}`}>
                ...
                <div className="grid grid-cols-3 sm:grid-cols-3 md:grid-cols-2 lg:grid-cols-2 divide-x divide-y divide-slate-100">
                  {[
      ...
      {configurandoCategorias && (
        <div className="fixed inset-0 bg-slate-900/50 flex items-end sm:items-center justify-center z-50 p-0 sm:p-4"
          ...
          <div className="bg-white rounded-t-2xl sm:rounded-2xl w-full sm:max-w-md shadow-2xl flex flex-col max-h-[90vh] sm:max-h-[80vh]"
            onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100 flex-shrink-0">
              <div className="flex items-center gap-2">
                <Settings className="w-4 h-4 text-[#4F46E5]" />
                <p className="text-sm font-semibold text-slate-800">Configurar categorías</p>
              </div>
