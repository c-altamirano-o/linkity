        ...
        <table className="w-full">
          <thead>
            <tr className="bg-slate-50 border-b border-slate-200">
              ...
              <th className="text-left text-[12.5px] font-medium text-slate-500 px-4 py-2.5">Módulos</th>
              <th className="text-left text-[12.5px] font-medium text-slate-500 px-4 py-2.5">Sucursales</th>
              <th className="text-left text-[12.5px] font-medium text-slate-500 px-4 py-2.5">Usuarios</th>
            </tr>
          </thead>
          <tbody>
            {filtrados.length === 0 ? (
                ...
                return (
                  <tr key={t.id} className="border-b border-slate-100 last:border-0 hover:bg-slate-50 transition-colors">
                    <td className="px-4 py-2.5">
                      <Link href={`/maestro/tenants/${t.slug}`} className="text-[13.5px] font-medium text-slate-800 hover:text-[#4F46E5]">
                        {t.name}
                      </Link>
                      <p className="text-[12.5px] text-slate-400">{[t.city, t.state].filter(Boolean).join(", ") || "Sin ubicación"}</p>
                    </td>
                    <td className="px-4 py-2.5 text-[13.5px] text-slate-600">{t.esquemaName ?? "Sin límite"}</td>
                    <td className="px-4 py-2.5">
                      <span className={`text-[12.5px] font-medium px-2 py-0.5 rounded-full ${cfg?.classes ?? "bg-slate-100 text-slate-400"}`}>
                        {cfg?.label ?? "Sin suscripción"}
                      </span>
                    </td>
                    <td className="px-4 py-2.5 text-[13.5px] text-slate-600">{t.modulosActivos}/{t.modulosTotal}</td>
                    <td className="px-4 py-2.5 text-[13.5px] text-slate-600">
                      {t.branchesCount}{t.esquemaMaxBranches !== null ? `/${t.esquemaMaxBranches}` : ""}
                    </td>
                    <td className="px-4 py-2.5 text-[13.5px] text-slate-600">{t.usersCount}</td>
                  </tr>
                );
              })
            )}
