...
export default function MaestroDashboardClient({ tenants }: { tenants: MaestroTenantRow[] }) {
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-lg overflow-hidden mb-5">
      {error && <p className="text-[12.5px] text-red-600 px-4 pt-2">{error}</p>}
      <table className="w-full">
        ...
        <tbody>
          {tenants.length === 0 ? (
            <tr>
              <td colSpan={5} className="px-4 py-6 text-center text-[13.5px] text-slate-400">
              ...
              return (
                <tr key={t.id} className="border-b border-slate-100 last:border-0 hover:bg-slate-50 transition-colors">
                  <td className="px-4 py-2.5">
                    <p className="text-[13.5px] font-medium text-slate-800">{t.name}</p>
                    <p className="text-[12.5px] text-slate-400">{[t.city, t.state].filter(Boolean).join(", ") || "Sin ubicación"}</p>
                  </td>
                  <td className="px-4 py-2.5 text-[13.5px] text-slate-600">{t.plan ?? "Sin plan"}</td>
                  <td className="px-4 py-2.5">
                  ...
                  <td className="px-4 py-2.5">
                    {puedeSuspender ? (
                      ...
                      <span className="text-[12.5px] text-slate-300" title="Sin acciones disponibles para este estatus">
                        Sin acciones
                      </span>
                    )}
                  </td>
