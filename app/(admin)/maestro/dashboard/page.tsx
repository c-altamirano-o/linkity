  ...
  return (
    <>
      ...
      <div className="flex-1 overflow-y-auto p-5">
        ...
        <div className="grid grid-cols-4 gap-3 mb-5">
          {metrics.map((m) => (
            <div key={m.label} className="bg-white border border-slate-200 rounded-lg p-3">
              <p className="text-[12.5px] text-slate-500 mb-1">{m.label}</p>
        ...
        {/* Tenants table */}
        <p className="text-[14.5px] font-medium text-slate-700 mb-2">Negocios registrados</p>
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="bg-white border border-slate-200 rounded-lg p-4">
            <p className="text-[14.5px] font-medium text-slate-700 mb-3">Negocios recientes</p>
            {data.recientes.length === 0 ? (
              <p className="text-[13.5px] text-slate-400">Todavía no hay negocios registrados.</p>
            ) : (
                ...
                {data.recientes.map((r) => (
                  <div key={r.id} className="flex items-start gap-2.5">
                    ...
                    <div>
                      ...
                      <p className="text-[12.5px] text-slate-400">Registrado el {formatFecha(r.createdAt)}</p>
                    </div>
                  </div>
                ))}
              </div>
          ...
          {/* Plans */}
          <div className="bg-white border border-slate-200 rounded-lg p-4">
            <p className="text-[14.5px] font-medium text-slate-700 mb-3">Distribución por plan</p>
            {data.distribucionPlanes.length === 0 ? (
              <p className="text-[13.5px] text-slate-400">Todavía no hay suscripciones registradas.</p>
            ) : (
              <div className="space-y-3">
                {data.distribucionPlanes.map((p, i) => (
                  <div key={p.plan} className="flex items-center gap-3">
                    ...
                    <p className="text-[13.5px] font-medium text-slate-700 w-4">{p.count}</p>
                  </div>
                ))}
              </div>
            )}
