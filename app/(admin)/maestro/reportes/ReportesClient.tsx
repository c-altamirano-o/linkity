    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-4 gap-3">
        <div className="bg-white border border-slate-200 rounded-lg p-3">
          <p className="text-[11px] text-slate-500 mb-1">Negocios totales</p>
          <p className="text-2xl font-medium text-slate-800">{data.totalNegocios}</p>
        </div>
        <div className="bg-white border border-slate-200 rounded-lg p-3">
          <p className="text-[11px] text-slate-500 mb-1">MRR actual</p>
          <p className="text-2xl font-medium text-slate-800">{formatMXN(data.mrrActual)}</p>
        </div>
        <div className="bg-white border border-slate-200 rounded-lg p-3">
          <p className="text-[11px] text-slate-500 mb-1">Ingreso promedio / negocio activo</p>
          <p className="text-2xl font-medium text-slate-800">{formatMXN(data.arpu)}</p>
        </div>
        <div className="bg-white border border-slate-200 rounded-lg p-3">
          <p className="text-[11px] text-slate-500 mb-1">Suspendidos / cancelados</p>
      ...
      <div className="bg-white border border-slate-200 rounded-lg p-4">
        <p className="text-[13px] font-medium text-slate-700 mb-1">Negocios nuevos por mes</p>
        <p className="text-[11px] text-slate-400 mb-4">Últimos 12 meses, según fecha de alta real de cada negocio</p>
        <div className="flex items-end gap-2 h-32">
          {data.altasPorMes.map((m) => (
            <div key={m.mes} className="flex-1 flex flex-col items-center gap-1.5">
              ...
              <p className="text-[9px] text-slate-400">{m.mes}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        {/* Ingresos por plan */}
        <div className="bg-white border border-slate-200 rounded-lg p-4">
          <p className="text-[13px] font-medium text-slate-700 mb-3">MRR por plan</p>
          {data.ingresosPorPlan.length === 0 ? (
            <p className="text-[12px] text-slate-400">Todavía no hay suscripciones activas.</p>
          ) : (
            <div className="space-y-3">
              {data.ingresosPorPlan.map((p, i) => (
                <div key={p.plan} className="flex items-center gap-3">
                  ...
                  <p className="text-[12px] font-medium text-slate-700 w-20 text-right">{formatMXN(p.mrr)}</p>
                </div>
              ))}
            </div>
          )}
        ...
        {/* Estados */}
        <div className="bg-white border border-slate-200 rounded-lg p-4">
          <p className="text-[13px] font-medium text-slate-700 mb-3">Estado actual de tus negocios</p>
